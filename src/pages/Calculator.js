...
const Calculator = () => {
  ...
  const proc = (x) => {
    console.log(x);
    return (parseFloat(x) / 100) * resolution;
  };
  ...
  return (
    <div className='fl-row fl-full-center' style={{height: '100%', width: '100%'}}>
      <div style={{height: '80%', width: '80%', textAlign: 'center'}}>
        <span style={{fontSize: '3vw'}}>VW to PX</span>
        <br/>
        <Link to='/px-to-vw' style={{marginTop: '0.5vw'}}><span style={{fontSize: '2vw'}}>PX to VW</span></Link>
            ...
            <textarea
              ...
              onChange={(e) => {
                ...
                while ((r = re.exec(s)) !== null) {
                  ...
                  z.push(
                    Math.round(
                      proc(r.toString().substring(0, r.toString().length - 2))
                    ) + 'px'
                  );
                }
